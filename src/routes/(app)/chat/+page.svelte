	<script lang="ts">
		...
		const viewer = useQuery(api.auth.getViewerIdentity, () => ($session.data ? {} : 'skip'));
		const roomsResponse = useQuery(api.chat.listRoomSummaries, () => ($session.data ? {} : 'skip'));
	...
	const messagesResponse = useQuery(api.chat.listMessages, () =>
