<script lang="ts">
	...
	$effect(() => {
		if (activeNav === 'club') clubOpen = true;
	});
	...
	<div class="flex min-h-screen flex-1 flex-col lg:flex-row">
		...
		>
			<div class="flex flex-col gap-1 px-5 py-5">
				<p class="text-xs tracking-[0.16em] text-muted-foreground uppercase">Curiosity Learning</p>
				<p class="text-sm font-medium text-foreground">Workspace</p>
			</div>
				...
				<nav class="flex flex-col gap-1">
					{#each topNavItems as nav (nav.key)}
						{#if nav.children?.length}
							<Collapsible bind:open={clubOpen}>
								<div
									class={cn(
								...
								>
									<a
										href={nav.href}
										class="flex flex-1 items-center gap-3 rounded-md px-3 py-2 text-left text-base font-medium"
									...
									<CollapsibleTrigger
										...
										aria-label={clubOpen ? `Collapse ${nav.label}` : `Expand ${nav.label}`}
									>
										<ChevronDownIcon
											class={cn('size-4 transition-transform', clubOpen ? 'rotate-180' : '')}
										/>
									</CollapsibleTrigger>
								</div>
		...
		<div class="flex min-h-screen flex-1 flex-col">
			...
			>
				<div class="flex w-full max-w-6xl flex-col gap-3 px-4 py-4 sm:px-6 lg:px-8">
					<div class="flex flex-wrap items-start justify-between gap-3">
						<div class="flex min-w-0 flex-1 items-start gap-2">
							...
							<div class="flex min-w-0 flex-col gap-1">
								<p class="text-xs tracking-[0.16em] text-muted-foreground uppercase lg:hidden">
									Curiosity Learning
								</p>
			...
			<footer
				class="fixed inset-x-0 bottom-0 flex justify-center border-t border-border bg-background/95 backdrop-blur lg:hidden"
			>
