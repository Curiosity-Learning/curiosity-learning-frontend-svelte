	...
	>
		...
		<div
			class={effectiveShowSideIllustration
				? `${edgeToEdgePanel ? 'min-h-[100dvh] gap-0 lg:min-h-screen lg:grid-cols-2' : 'min-h-[calc(100dvh-2rem)] gap-6 pt-6 sm:pt-8 lg:min-h-[calc(100vh-8rem)] lg:grid-cols-2 lg:gap-14'} grid items-stretch`
				: `${appFrame ? 'min-h-0' : edgeToEdgePanel ? 'min-h-[100dvh] lg:min-h-screen' : 'min-h-[calc(100dvh-2rem)] pt-6 sm:pt-8 lg:min-h-[calc(100vh-8rem)]'} flex flex-col`}
		>
			<div
				class={effectiveShowSideIllustration
					? `onboarding-shell-panel ${edgeToEdgePanel ? 'onboarding-shell-panel--edge' : ''} flex w-full min-w-0 ${edgeToEdgePanel ? 'min-h-[100dvh] max-w-none px-0 lg:min-h-screen' : 'min-h-[calc(100dvh-2rem)] px-1 lg:min-h-[calc(100vh-8rem)]'} flex-1 flex-col ${desktopContentScrollable && !edgeToEdgePanel ? 'lg:max-h-[calc(100vh-8rem)]' : ''}`
					: `onboarding-shell-panel ${appFrame ? 'onboarding-shell-panel--app min-h-0 max-w-none px-0 pb-0' : edgeToEdgePanel ? 'onboarding-shell-panel--edge min-h-[100dvh] px-0 lg:min-h-screen' : 'mx-auto min-h-[calc(100dvh-2rem)] px-1 lg:min-h-[calc(100vh-8rem)]'} flex w-full min-w-0 flex-1 flex-col`}
			>
