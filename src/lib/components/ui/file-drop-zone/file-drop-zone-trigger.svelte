...
>
	...
	{:else}
		<div
			class={cn(
				'flex h-48 flex-col place-items-center justify-center gap-2 rounded-lg border border-dashed p-6 transition-all group-aria-disabled/file-drop-zone-trigger:opacity-50 group-aria-disabled/file-drop-zone-trigger:hover:cursor-not-allowed',
				triggerState.rootState.isDragActive
					? 'border-primary bg-accent/35 shadow-xs'
					: 'hover:bg-accent/25 hover:cursor-pointer'
			)}
		>
			<div
				class={cn(
					'text-muted-foreground flex size-14 place-items-center justify-center rounded-full border border-dashed transition-all',
					triggerState.rootState.isDragActive
