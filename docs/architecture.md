...
- `/media/[assetId]`: authenticated app-level media fetch route. This is the canonical media read path for UI code so consumers do not depend directly on raw S3 URLs. When secure media delivery env is configured, the route issues CloudFront signed cookies and redirects to the media CDN URL. Until that infra is live, it falls back to the resolved ready-file URL for development.
- `/settings/media-upload-dev`: authenticated developer-facing upload test surface for the shared media pipeline. It is intentionally built from existing app primitives so upload mutations, direct-to-S3 descriptors, finalize/retry/cancel actions, and status polling can be exercised manually without adding one-off UI components elsewhere in the product.
