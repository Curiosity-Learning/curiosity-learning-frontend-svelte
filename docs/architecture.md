...
- **Shared media upload pipeline**: `src/convex/media.ts`, `src/convex/mediaPipeline.ts`, and `src/convex/mediaProcessing.ts` define a constraint-based upload state machine backed by Convex file storage. Clients create a draft upload, POST directly to the generated upload URL, then finalize with the returned `Id<"_storage">`. A Node-side internal action then validates file bytes, enforces size plus optional duration limits, compresses media, runs AWS Rekognition screening, and only marks the asset usable when it is both `ready` and `approved`. Raw upload blobs are stored separately from the final processed asset so rejected uploads never become normal feature attachments. File URLs stay ephemeral via `ctx.storage.getUrl(...)`. Feature ownership stays in domain tables, and feature-level rules like attachment count remain outside the shared uploader. See [ADR-012](adr/012-shared-media-upload-pipeline.md) and [ADR-013](adr/013-media-safety-and-compression-pipeline.md).
...
- **Feed update card surface**: `/feed/my-clubs` renders updates through `src/lib/components/app/feed/update-card.svelte`. The related project displays as a small default `Badge` pill, and the related prompt/question displays as orange `type-sm-bold` text above update content. Project update attachments now reference approved `mediaAssets` rows instead of raw storage IDs, but rendering those attachments in the feed UI remains a follow-up iteration.
- **Media pipeline sandbox route**: `/settings/media-upload-dev` is an authenticated internal test surface for the shared uploader. It exercises the real `media.beginUpload -> storage POST -> media.finalizeUpload` flow, displays live `mediaAssets` state, and helps debug compression/moderation processing without introducing feature-specific product UX.
