...
- Profile avatars and club intro videos also attach through `mediaAssets` (`profiles.profileImageMediaAssetId`, `clubs.videoMediaAssetId`). Single-media routes should receive signed media URLs inline from their server load and render plain `<img>` / `<video>` elements. `coverPhotoUrl` and `videoUrl` remain legacy/public fallbacks, not the primary read path for uploaded private media.
