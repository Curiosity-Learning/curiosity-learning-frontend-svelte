...
- 2026-03-30: Fixed the onboarding sign-up and post-signup flows to use SvelteKit's `replaceState(...)` helper instead of raw `history.replaceState(...)`, and guarded post-signup completion against duplicate redirects. This addresses the `undefined is not an object (evaluating 'app.hash')` runtime failure when finishing onboarding.
