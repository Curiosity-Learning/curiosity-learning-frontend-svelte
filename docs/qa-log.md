...
- 2026-03-29: Wrapped the `/api/auth/[...all]` SvelteKit proxy handler so aborted Better Auth session requests return quietly instead of surfacing as `[500] GET /api/auth/get-session` during cancelled navigations in development.
- 2026-03-29: Stopped `src/routes/auth/+layout.server.ts` from redirecting authenticated `/auth/sign-up` requests to `next`, so OAuth callback flows can stay on the signup step and show blocked/existing-account messaging instead of bouncing into the join-club details page.
- 2026-03-29: Added real username-or-email resolution for auth recovery flows by looking up profile usernames before calling Better Auth email endpoints, so sign-in and reset-password now both accept the username chosen during post-signup setup.
- 2026-03-29: Reintroduced lightweight public-page i18n with English and Dutch message bundles, shared public auth/onboarding language switching, and translated copy across the public onboarding/auth flow without adding a heavier translation library.
- 2026-03-29: Extended the lightweight i18n pass to the cookie-consent banner, privacy/terms/cookie page chrome, settings policy links, and reusable upload-related fallback errors so those user-visible surfaces no longer depend on hard-coded English.
...
- 2026-04-11: Refined `/club/[clubId]` dashboard UI to match the updated club-home design with horizontal upcoming-session and project rails, compact shared session cards (tags + attendees only on dashboard), learner row/grid presentation, and empty-state header actions that reuse existing shared Svelte components.
- 2026-04-11: Polished `/club/[clubId]` visual parity by switching the page body to a Figma-like light-gray background and rendering section empty states with existing asset illustrations (`no_session_found.png`, `no_projects_found.png`, `no_learners_found.png`) through the shared `HomeEmptyCard`.
- 2026-04-13: Matched app-shell/sidebar styling closer to the club-dashboard Figma (white shell background, orange active-strip + selected row treatment) and updated club-home empty states to plain centered illustration sections (`Sessions`, `Projects`, `Learners`) with right-aligned add/invite actions.
...
- 2026-04-27: Cleaned repo hygiene before new feature work: removed macOS AppleDouble metadata files, ignored future `._*` files, standardized on npm by removing `pnpm-lock.yaml`, renamed the package from the migration-era name, refreshed stale `/app/*` docs/tests to the current `(app)` route group URLs, and updated the docs index for ADR-012 through ADR-015.
